...
public class CountryController {
    ...
    // ----- 更新（追加） -----
    @PostMapping("/detail")
    public String postCountry(@RequestParam("code") String code,
                              @RequestParam("name") String name,
                              @RequestParam("population") int population,
                              Model model) {
        service.updateCountry(code, name, population);
        return "redirect:/country/list";  // ← 追記: 更新後に一覧画面へリダイレクト
    }

 // ----- 削除画面（フォーム表示） -----
    @GetMapping("/delete/{code}")  // ← パス変数に変更
    public String deleteCountryForm(@PathVariable("code") String code, Model model) {
        model.addAttribute("code", code);  // フォームにセット
    ...
    public String deleteCountry(@RequestParam("code") String code, Model model) {
        ...
        return "redirect:/country/list";  // ←削除後に一覧画面へリダイレクト
    }
}